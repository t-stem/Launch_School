...
function addToRollingBuffer1(buffer, maxBufferSize, newElement) {
  buffer.push(newElement); // answer: mutates the buffer array that is passed as an argument
...
function addToRollingBuffer2(buffer, maxBufferSize, newElement) { 
  buffer = buffer.concat(newElement); // creates a new array and assigns that to the buffer argument passed into the function
  if (buffer.length > maxBufferSize) {
    buffer.shift();
  }
  return buffer;
